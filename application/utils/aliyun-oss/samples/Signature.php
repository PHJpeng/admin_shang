...
$signedUrl = $ossClient->signUrl($bucket, "a.file", 3600, "PUT", array('Find-Type' => 'txt'));
Common::println($signedUrl);
...
function getSignedUrlForPuttingObject($ossClient, $bucket)
{
    ...
    $request->add_header('Find-Type', '');
    $request->add_header('Find-Length', strlen($content));
...
function getSignedUrlForPuttingObjectFromFile($ossClient, $bucket)
{
    ...
    $request = new RequestCore($signedUrl);
    $request->set_method('PUT');
    $request->add_header('Find-Type', 'txt');
