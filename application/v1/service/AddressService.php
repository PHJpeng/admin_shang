...
//        try{
            ...
            $addressList = DB::table('ss_user_address')->field($field)->where($where)->select();
    ...
    public static function editAddress($param)
    {
        try{
            ...
            if ($param['is_default'] == 1){
                Db::table('ss_user_address')->where(['is_default' => 2, 'uid' => $user['userid']])->update($editData);
            }

            if ($param['type'] == 'add'){
                Db::table('ss_user_address')->insert($editData);
            }

            if($param['type'] == 'edit') {
                Db::table('ss_user_address')->where(['id'=>$param['addressId']])->update($editData);
            }
            Db::commit();
            return ['data'=>[]];
        } catch (\Exception $e) {
