    ...
    public static function enterCode($param)
    {
        try{
            ...
            if($codeHas){
                ...
                $add_member['head_img'] = '/upload/20200817/57d1391ef78269a7df9df736a0fa8cea.png';
...
//            }
    ...
    public static function taskend($param)
    {
        try{
            ...
            if ($progress == 5 || $progress == 11 || $progress == 22) {
                if ($level != 1) {
                    Db::commit();
                    return ['data' => []];
                }
                ...
                switch ($progress) {
                    case 5:
                        ...
                        break;
                    case 11:
                        $orthor_sign_money = 10;
                        break;
                    case 22:
                        $orthor_sign_money = 20;
                        break;
                    default:
                        $orthor_sign_money = 5;
                }

                $res = Db::table('ss_store')->where(['uid' => $user['userid']])->setInc('get_coin',$orthor_sign_money);
                if(!$res){
                    // 回滚事务
                    Db::rollback();
                    return ['msg'=>'网络繁忙，请稍后再试!'];
                ...
                $data['username'] = $user_info['username'];
                $data['account'] = $user_info['account'];
                $data['content'] = '完成指定数量任务奖励'.$orthor_sign_money.'个币';
                $data['inc_money'] = $orthor_sign_money;
                $data['over_money'] = $over_moeny;
                $data['mtype'] = 'get_coin';
                $data['create_time'] = time();
                $data['datestr'] = date('Ymd');
                ...
                $add = add_money_detail($user['userid'],'task','完成指定数量任务奖励'.$orthor_sign_money.'个币',$orthor_sign_money,'get_coin');
                if(!$add){
                    // 回滚事务
                    Db::rollback();
                    return ['msg'=>'网络繁忙，请稍后再试!'];
